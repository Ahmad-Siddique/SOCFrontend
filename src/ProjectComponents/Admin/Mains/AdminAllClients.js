...
const AdminAllClients = () => {
  ...
  const updateData = (id) => {
    navigate("/alladmins/updateclient/" + id);
  };

  const DeleteUser = async (id) => {
    const data = await axios.post(
      "http://localhost:5000/api/deleteclient/" + id
    );
    ...
    <div>
      ...
      <div>
        <div className="d-flex align-items-center mb-3">
          <div>
            <h1 className="page-header mb-0">ALL CLIENTS</h1>
          </div>
        </div>
                ...
                <table className="table table-hover text-nowrap">
                  <thead>
                    <tr>
                      ...
                      <th className="border-top-0 pt-0 pb-2">Contact Person</th>
                      <th className="border-top-0 pt-0 pb-2">Email</th>
                      <th className="border-top-0 pt-0 pb-2">Mobile No</th>
                      <th className="border-top-0 pt-0 pb-2">Update</th>
                      <th className="border-top-0 pt-0 pb-2">Delete</th>
                    </tr>
                  </thead>
                  <tbody>
                    {clientdata === [] && <div>No data to show</div>}
                    {clientdata &&
                      clientdata.map((e) => {
                        return (
                          <tr>
